...
struct TrajectoryCanvas: View {
    ...
    private func drawPFConfidence(
        ...
        let sampleCount = min(max(points.count - 1, 0), confidence.count)
        guard sampleCount > 0 else { return }
        let stride = max(1, Int(ceil(Double(sampleCount) / 10.0)))
        for sampleIndex in 0..<sampleCount
        where sampleIndex % stride == 0 || sampleIndex == sampleCount - 1 {
            let item = confidence[sampleIndex]
            let center = screenPoint(
                points[sampleIndex + 1],
                bounds: bounds,
                size: size
            )
