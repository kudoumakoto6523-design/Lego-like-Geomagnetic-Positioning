...
2. 导入新采集并选择第一次建立的地磁图。
3. 检查“真实路线坐标”的点序提示；重复点、近 `180°` 原路折返及超出磁图范围的坐标必须先修正。
4. 点击“开始计算”。
5. 白线只是按输入坐标顺序连接的评估路线，不是传感器自动还原结果；青线是 PDR，黄色虚线是 PF 地磁匹配结果，绿色区域是 PF 粒子置信范围。
...
6. 房间自由定位会自动识别任意角度的直行段与转弯区域，在直行时抑制航向噪声、转弯时立即释放；不会默认吸附到 `90°`。
7. PF 每步使用连续位移约束抑制长边上的侧向模式跳跃，并在重采样时保留最多四个分离空间候选；粒子只能存在于磁图支持范围内。
...
- 真实路线格式、重复点、原路折返和磁图覆盖范围校验；白线含义在界面中明确说明。
...
- 正方形房间磁图：8 组独立建图采集合并，56 个有效网格，覆盖率约 `88%`，质量评级“优秀”。
- 独立自由路线 `localization_test_6`：PF 横向误差 `0.13 m`、沿程误差 `0.19 m`、终点误差 `0.16 m`、路线长度比例 `103%`、转角误差 `42.9°`。
- 该结果属于已知起点和当前小房间磁图的阶段性实测，不代表未知起点或跨房间泛化精度。
