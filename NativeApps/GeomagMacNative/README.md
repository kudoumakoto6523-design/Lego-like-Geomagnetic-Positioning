...
新版 iPhone/HarmonyOS 包如果包含 `SpatialEvents.csv` 中的两个或更多坐标锚点，macOS 会优先使用锚点建图：自由采集会自动用锚点补全路线并由首段推导初始航向；建图按 PDR 累计距离分配采样位置，并生成默认 `0.25 m` 二维网格。每格保存房间坐标系 `Bx/By/Bz`、模长、方差、样本数、采集方向数和可计算的磁场梯度。
...
6. 默认航向采用任意角度的自适应直线软校正；弧线和转弯立即释放。固定角度吸附只保留为特殊场景参数。
7. 网格 PF 在重采样时保留最多四个分离空间候选，避免长边磁场相似时过早锁定错误位置；粒子只能存在于磁图支持范围内。
